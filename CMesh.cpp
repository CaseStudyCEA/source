 ...
 */
/***********************************************************************/
void CMesh::draw(const D3DXMATRIXA16* matView,const D3DXMATRIXA16* matWorld)
{
	...
	if(_pEffect == NULL)
	{
		drawEffectOff(matView,matWorld);
		return;
	}

	_pEffect->draw(this,matView, matWorld);
}
 ...
 */
/***********************************************************************/
void CMesh::drawEffectOff(const D3DXMATRIXA16* matView,const D3DXMATRIXA16* matWorld)
{
	/////////////////////////
	//	マトリクス変換
	D3DDEVICE->SetFVF(FVF_VERTEX_3D);	// 頂点フォーマットの設定
	D3DDEVICE->SetTransform(D3DTS_PROJECTION,CSCREEN->getProjPtr());	//ビュー座標変換
	D3DDEVICE->SetTransform(D3DTS_VIEW, matView);						//カメラ座標変換
